...
AntsReview (See Figure \ref{fig:contracts}) is the core of the smart contracts written in \emph{Solidity}\footnote[6]{\emph{Solidity}.\textsc{URL:}https://solidity.readthedocs.io/en/v0.6.8.}, an object-oriented programming language for writing smart contracts that runs on the Ethereum Virtual Machine (EVM), deployed on Ethereum Rinkeby's Testnet\footnote[7]{\emph{}.\textsc{URL:}https://solidity.readthedocs.io/en/v0.6.8.}.
...
\begin{itemize}
  ...
  \item to specify the amount of Ether for the reward, to be trasfered from Alice to the contract.
\end{itemize}
...
\newline If Alice doen't receive any peer-review, she can cancel the AntReview with the function \emph{cancelAntReview()}, specifying the AntReview Identifier, and the contract will transfer the reward for the AntReview back to Alice.
...
\newline zkANTS, is a wrapper of ANTS that will be used inside the protocol to allow private transactions on mainnet among the agents, preserving their anonimity as well as the amount of the AntReview reward and the contributions by the Anters (members of Ants-Review Community).
\newline zkANTS will be implemented via AZTEC Protocol \cite{AZTEC}, that uses a cryptographic engine \emph{ACE.sol}, a contract responsible for for validating the set of AZTEC zero-knowledge proofs and performing any transfer instructions involving AZTEC notes, minted into a zkAsset, that can be converted into ERC20 tokens. In order to implement a zkAsset called zkANTS, \emph{zkAsset.sol}, a contract implementation of a confidential token, that follows the EIP-1724 standard \footnote{\url{https://github.com/ethereum/EIPs/issues/1724}} will be used as template to build an AZTEC-compatible asset.
\newline To optimize and scale transactions, AZTEC Protocol ZK-SNARKs, will be extended in the future to PLONK \cite{PLONK} to collapse the gas costs of private transactions on mainnet.
