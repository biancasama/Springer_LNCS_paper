...
An interesting aspect of the protocol is the double function of an AntReview that is a the same time a bounty and a pool to stake ERC20 tokens like Dai\footnote[14]{\emph{Dai}.\textsc{URL:}https://docs.makerdao.com/smart-contract-modules/dai-module/dai-detailed-documentation.} . Therefore, the duration of peer-reviews consents to connect the protocol to De-Fi services with the possibility of accruing interest over time via MakerDAO DSR (Dai Saving Rate)\footnote[16]{\emph{Pot}.\textsc{URL:}https://docs.makerdao.com/smart-contract-modules/rates-module/pot-detailed-documentation.} or Compound\footnote[17]{\emph{Compound}.\textsc{URL:}https://compound.finance.} to cite a few. 
In particular, we are considering to implement another ERC20 token, a pool token that represents the accrued interest of the Anters' stake into AntReviews.
This token doesn't need to be locked in the protocol and it can be traded, sold, or held as the Anter desires.
...
\newline Finally, a protocol upgrade under investigation would allow Anters to validate peer-reviews via an upvote/downvote system inspired by Discover \footnote[18]{\emph{Discover.sol}.\textsc{URL:}https://github.com/dap-ps/discover/blob/master/contracts/Discover.sol.} (Status \footnote[19]{\emph{Status}.\textsc{URL:}https://status.im/.}), that automatically will pay out the reward to peer-reviewers based on the votes associated with their peer-reviews.
