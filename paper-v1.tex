...
\textbf{ANTS} is the native token used in the protocol that inherits the \textbf{ERC20PresetMinterPauser} contract from OpenZeppelin. It's linked to AntsReview and to AZTEC Protocol to be wrapped into \emph{zkANTS} to implement private \emph{ANTS} transactions on Ethereum.
...
\textbf{De-Fi} represents the integration with De-Fi services like \emph{Dai} and \emph{Chai}, to be used as tokens in the protocol.
\textbf{OpenZeppelin} is the library used by the protocol for secure contract development, with the modules listed.
\end{figure}
