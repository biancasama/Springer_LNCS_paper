...
\emph{AntsReview}, which manages access management and the core system (see Figure \ref{fig:contracts}, a-b; f; h).
\emph{Tokenomics}, which manages the incentive mechanism of the system (see Figure \ref{fig:contracts}, c-e; h}).
\emph{Privacy}, which maintains the anonymity of the system via AZTEC Protocol (see Figure  \ref{fig:contracts}, d).

\subsubsection{AntsReview.}
AntsReview (see Figure \ref{fig:contracts}a) is the core of the smart contracts written in \emph{Solidity}\footnote[6]{\emph{Solidity}.\textsc{URL:}https://solidity.readthedocs.io.}, an object-oriented programming language for writing smart contracts that runs on the Ethereum Virtual Machine, deployed on Ethereum Rinkeby's Testnet\footnote[7]{\emph{Rinkeby Testnet}.\textsc{URL:}https://rinkeby.etherscan.io.}. AntsReview implements a bounty-like system where Alice (issuer) can issue an AntReview with the function \emph{issueAntReview()}.
...
\newline ANTS (see Figure \ref{fig:contracts}, c), is the primary protocol ERC20 token and can be staked into an AntReview.
...
\newline zkANTS (see Figure \ref{fig:contracts}, d), is a wrapper of ANTS that will be used inside the protocol to allow private transactions among the agents, preserving their anonymity as well as the amount of the AntReview reward and the contributions by the Anters (members of Ants-Review Community).
