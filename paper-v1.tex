...
\item \emph{transparency of the records}: all reviews should be open access to the community ('open report') to prevent waste of knowledge and disrupt the secretive system of black boxes of peer reviews in journals that centralize all the reports 
...
\begin{itemize}
\item \emph{AntsReview}, which manages access management and the basic system.
\item \emph{Tokenomics}, which manages the system of incentivization mechanism by integrating ideas from DeFi (Decentralized Finance) and Quadratic Voting.
...
\newline It implements a bounty-like system where an issuer (author or editor) is asked a series of actions, to ensure transparency and robustness, regarding the AntReview that s/he is creating:
\begin{itemize}
...
\newline The AntReview after being validated is then accepted by the Issuer and paid between Peer-Reviewers.
...
\newline In Ethereum \cite{Ethereum} the concept of immutability is achieved via the blockchain and proof of work consensus algorithm by a chain of timestamped block hashes that are secured by miners competing computations.
