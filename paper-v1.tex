...
\begin{enumerate}
    \item \textbf{Ants-Review process: scenario 1 (journal-dependent)}: 1. The author submits a manuscript to the journal. 2. If the papers passes the stages of editorial decision (appropriateness of style and coherence with the journal's scope). 3. The editor issues a bounty (\emph{bounty issuance}) for the paper review. Fulfillment rules are set in a smart-contract. 4. The bounty prize could correspond to part of the article submission fees and article publication fees requested to the authors by the journal. 5. Reviews fulfilling the smart-contract requirements are validated by the journal editors. 6. When the reviews are accepted (\emph{bounty fulfillment}), reviewers are awarded a token called \emph{ANT}.
    \item \textbf{Ants-Review process: scenario 2 (community-driven)}: 
...
\begin{enumerate}
\item The author submits a manuscript to the pre-print service or to an hypothetical Ants-Review platform.
...
\item Reviews fulfilling the smart\-contract requirements are validated by the Ants-Review community of contributors.
...
\newline Ant [see \ref{lst:code}] is implemented by inheriting \emph{ERC20PresetMinterPauser.sol} from OpenZeppelin Library with name \emph{Ant} and symbol \emph{ANT}.
...
\begin{itemize}
  ...
  \item via private transactions allowed by AZTEC Protocol security layer via zkAnt. Future developments will allow to leverage on PLONK\cite{PLONK}proofs, a universal ZK-SNARK construction, to reduce gas costs and improve scalability.
\end{itemize}
...
\newline To avoid that, Ants-Review will implement zkAnt, an Ant token wrapped into AZTEC Protocol. In the future zkAnt could leverage on zkSNARKS and PLONK, allowing private transactions in the system to maintain the privacy and anonymity of Peer-Reviewers.
...
Some criticism towards giving a monetary reward to reviewers tackle the fact that an extrinsic motivation (such as a token system) might decrease the intrinsic motivation that drives researchers to perform scientific contributions. Some experimental studies seem indeed to suggest that reviewers are more interested in reputational forms of recognition \cite{Warne-RewRev}. However, we think that engaging the whole community towards the peer-review process and rewarding each actor in it could diminish these effects.

\subsection{A new community-driven standard?}
A change is already happening in the publishing industry, especially with new born publishers opening up the review process (BioMed Central, ELife, Frontiers, PeerJ, F1000 Research). As Tennant points out \cite{Tennant2017-F1000R}, if also pre-print platforms integrate peer-review into their platforms, the dissociation of initial scientific dissemination and scientific validation will force publishing industry to adapt in order to justify their added value.
...
\bibliography{ref1-intro.bib,ref2-peerreview.bib,ref2-relatedwork.bib, ref3-system.bib,ref4-discuss.bib}
