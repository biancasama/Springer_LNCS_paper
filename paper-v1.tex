...
\newline (\textbf{AntsReviewRoles}) is the module inherited by AntsReview, that manages the access control of the protocol, adding and removing issuers and peer-reviewers.
\newline (\textbf{ANTS}) is the native token used in the protocol that inherit the ERC20PresetMinterPauser library from OpenZeppelin, is linked to AntsReview and to Aztec Protocol to be wrapped into zkANTS to implement private transactions on Ethereum.
\newline (\textbf{IPFS}) represent the integration with IPFS to upload the paper, requirements and peer-reviews and store the hash as proof of existence into AntsReview.
\newline (\textbf{De-Fi}) represent the integration with De-Fi services like Dai and Chai, to be used as token in the protocol. (\textbf{OpenZeppelin}) is the library used by the protocol for secure contract development, with the modules listed. 
\end{figure}
