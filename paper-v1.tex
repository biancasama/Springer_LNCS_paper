...
\medskip
\small
\textbf{AntsReview} is the core of the protocol, it implements a bounty system with the functions listed.
...
\textbf{ANTS} is the native token used in the protocol that inherits the \textbf{ERC20PresetMinterPauser} library from OpenZeppelin, it is linked to AntsReview and to Aztec Protocol to be wrapped into \emph{zkANTS} to implement private \emph{ANTS} transactions on Ethereum.
\textbf{IPFS} represent the integration with IPFS to upload the paper, requirements and peer-reviews and store the hash as \emph{proof of existence} into AntsReview.
\textbf{De-Fi} represent the integration with De-Fi services like \emph{Dai} and \emph{Chai}, to be used as token in the protocol. (\textbf{OpenZeppelin}) is the library used by the protocol for secure contract development, with the modules listed.
\end{figure}
